...
def to_limbs(text, base_digits=BASE_DIGITS):
    ...
    while num > 0:
        limb_list.append(num % BASE)
        num = num // BASE

    if len(limb_list) == 0:
        limb_list.append(0)

    limb_list = np.array(limb_list, dtype=np.int64)

    return (sign, limb_list)
...
def from_limbs(sign, limbs, base_digits=BASE_DIGITS):
    ...
    temp=0
    for i in limbs:
        new_limb.append((i+temp)%base)
        temp=(i+temp)//base
    if temp>0:
        new_limb.append(temp)
    num=0
    j=0
    for i in new_limb:
        num+=i*(base**j)
...
def multiply_transform(a, b, engine):
    ...
    c = np.rint(c.real).astype(np.int64)
...
def multiply(text_a, text_b, method):
    ...
    if method == "dft":
        engine = DFTAnalyzer()
        raw, N = multiply_transform(limbs_a, limbs_b, engine)
        ...
        raw = multiply_schoolbook(limbs_a, limbs_b)
        N = len(raw)

    else:
        raise ValueError("Unknown multiplication method")

    product = from_limbs(sign, raw)
...
def run_single(path, method, out_dir):
    ...
    # --------------------------------------------------
    # 1. Read the two decimal operands
    # --------------------------------------------------
    text_a, text_b = read_operands(path)

    # --------------------------------------------------
    # 2. Multiply using our implementation
    # --------------------------------------------------
    ...
    # --------------------------------------------------
    # 3. Verify using Python big integers
    #    THIS is the only place where big-int multiplication
    #    is allowed.
    # --------------------------------------------------
    ...
    if verdict == "MISMATCH":
        raise AssertionError(
            "Verification failed: computed product does not match "
            "Python's integer multiplication."
        )
    ...
    # --------------------------------------------------
    # 5. Prepare report information
    # --------------------------------------------------
    digits_a = len(text_a.lstrip("+-"))
    digits_b = len(text_b.lstrip("+-"))
    digits_product = len(product.lstrip("-"))

    limb_count_a = len(limbs_a)
    limb_count_b = len(limbs_b)
    ...
    lines = [
        ...
        "digits_b: %d" % digits_b,
        "base: %d" % BASE,
        "limbs_a: %d" % limb_count_a,
        "limbs_b: %d" % limb_count_b,
        "transform_length: %d" % N,
        "product_digits: %d" % digits_product,
