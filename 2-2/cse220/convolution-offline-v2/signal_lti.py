...
class DiscreteSignal:
    ...
    def __init__(self, start_time, end_time):
        ...
        self.signal=np.zeros(end_time-start_time+1)
    ...
    def get_value_at_time(self, t):
        if t<self.start_time or t>self.end_time:
            ...
            return self.signal[t-self.start_time]
    ...
    def shift(self, k):
        ...
        for t in self.times():
            shifted.set_value_at_time(t + k, self.get_value_at_time(t))
        return shifted
    ...
    def add(self, other):
        new_start_time=min(self.start_time,other.start_time)
        new_end=max(self.end_time,other.end_time)
        new_sig=DiscreteSignal(new_start_time,new_end)
        for t in new_sig.times():
            new_sig.set_value_at_time(t,self.get_value_at_time(t)+other.get_value_at_time(t))
        return new_sig
    ...
    def nonzero_samples(self, tolerance=1e-12):
        return [(t,self.get_value_at_time(t)) for t in self.times() if abs(self.get_value_at_time(t))>tolerance]
...
class LTISystem:
    ...
    def get_response_components(self, input_signal):
        ...
        for k, val in input_signal.nonzero_samples():
            ...
            component = shifted_h.multiply(val)
            components.append((k, component))
        return components
        #
    ...
    def output_by_superposition(self, input_signal):
        ...
        temp = DiscreteSignal(start, end)
        for _, component in components:
            temp = temp.add(component)
        return temp
    ...
    def get_contributions_at_time(self, input_signal, n):
        contributions = []
        for t_idx,value in input_signal.nonzero_samples():
            h_value = self.impulse_response.get_value_at_time(n - t_idx)
            product = value * h_value
            if abs(product) > 1e-12:
                contributions.append((t_idx, value, h_value, product))
    ...
    def output_at_time(self, input_signal, n):
        ...
        output_value = sum(product for _, _, _, product in contributions)
        return output_value
