...
def make_signal(start_time, end_time, values):
    ...
    if len(values) != len(signal):
        raise ValueError("Number of values must match the signal length")

    for index, value in enumerate(values):
        signal.set_value_at_time(start_time + index, value)
    return signal
...
def signal_from_samples(start_time, end_time, samples):
    signal = DiscreteSignal(start_time, end_time)
    for time_index, value in samples.items():
        signal.set_value_at_time(time_index, value)
    return signal
...
def impulse_moving_average(length):
    ...
    for time_index in signal.times():
        signal.set_value_at_time(time_index, 1.0 / length)
    return signal
...
def max_absolute_difference(first_signal, second_signal):
    ...
    for time_index in range(start_time, end_time + 1):
        difference = abs(first_signal.get_value_at_time(time_index) - second_signal.get_value_at_time(time_index))
