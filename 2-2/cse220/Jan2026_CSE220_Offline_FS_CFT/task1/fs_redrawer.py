...
class FourierEpicycles:
    def __init__(self, t, signal, n_harmonics):
        ...
        self.t=t  #samples of t axis from 0 to 2PI divided into 1000
        self.signal=signal
        self.N=n_harmonics
        self.T=t[-1]-t[0]  #t = 0 to 2pi er 1000 vag sample,so 1st element is 0 and
                        #last element t[-1] is 2PI
        self.omega=2*np.pi/self.T  #hence omega is 1 (2PI/2PI) ,T=2PI
        self.coeffs={}
        #it will contain cn of corresponding N
    ...
    def approximate(self, t):
        ...
        for n in range(-self.N, self.N + 1):
          x_t += self.coeffs[n] * np.exp(1j * n * self.omega * t)
...
if __name__ == "__main__":
    ...
    fs = FourierEpicycles(t, z, n_harmonics=N_HARMONICS) #z is signal=np.array([2 + 3j,4 + 1j,5 - 2j])
    fs.calculate_all_coefficients()

    save_outputs(fs, z, comparison_path, gif_path, num_frames=240)
#terminal: python fs_redrawer.py svgs/heart.svg 150 heart_comparison.png heart_epicycles.gif
#150 is N, the more harmonics the better perfect shape