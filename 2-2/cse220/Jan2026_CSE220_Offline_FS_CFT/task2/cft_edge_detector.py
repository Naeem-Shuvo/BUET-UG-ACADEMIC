...
class CFT2D:
    ...
    def compute_cft(self):  
        ...
        x_cos = np.trapezoid(  
            self.I[:, None, :] * cos_ux[None, :, :],  
            self.x,  
            axis=2  
        )  
 
        x_sin = np.trapezoid(  
            self.I[:, None, :] * sin_ux[None, :, :],  
            self.x,  
            axis=2  
        )  
        ...
        real = (  
            np.trapezoid(  
                x_cos[None, :, :] * cos_vy[:, None, :],  
                self.y,  
                axis=2  
            )  
            -  
            np.trapezoid(  
                x_sin[None, :, :] * sin_vy[:, None, :],  
                self.y,  
                axis=2  
            )  
        )  
        ...
        imag = -(  
            np.trapezoid(  
                x_sin[None, :, :] * cos_vy[:, None, :],  
                self.y,  
                axis=2  
            )  
            +  
            np.trapezoid(  
                x_cos[None, :, :] * sin_vy[:, None, :],  
                self.y,  
                axis=2  
            )  
        )  
    ...
    def plot_magnitude(self):
        ...
        plt.imshow(  
            log_magnitude,  
            cmap='gray',  
            extent=[self.u[0], self.u[-1], self.v[-1], self.v[0]]  
        )  
        plt.title("Log-Magnitude Fourier Spectrum")  
        plt.xlabel("u")  
        plt.ylabel("v")  
        plt.show()  
...
class InverseCFT2D:
    ...
    def reconstruct(self):  
        ...
        image : 2D numpy array of shape (len(self.y), len(self.x))  
            ...
            that after a high-pass filter this is NOT a valid image on its  
            own (it will contain negative values, since the DC/  
        ...
        cos_vy = np.cos(  
            2 * np.pi * self.y[:, None] * self.v[None, :]  
        )  
        ...
        v_real = np.trapezoid(  
            self.real[None, :, :] * cos_vy[:, None, :]  
            - self.imag[None, :, :] * sin_vy[:, None, :],  
            self.v,  
            axis=2  
        )  
  
        cos_ux = np.cos(  
            2 * np.pi * self.x[:, None] * self.u[None, :]  
        )  
        ...
        v_imag = np.trapezoid(  
            self.real[None, :, :] * sin_vy[:, None, :]  
            + self.imag[None, :, :] * cos_vy[:, None, :],  
            self.v,  
            axis=2  
        )  
   
        image = np.trapezoid(  
            v_real[:, :, None] * cos_ux.T[None, :, :]  
            - v_imag[:, :, None] * sin_ux.T[None, :, :],  
            self.u,  
            axis=1  
        )  
